...
class CobolNodeFactory {
  ...
  /**
   * Create the constructor / entry point of the program. It is added as a function with the name of
   * program to the list.
   *
  ...
  void createConstructor(
    ...
    FrameSlot frameSlot =
        frameDescriptor.findOrAddFrameSlot(
            CobolProgramStateNode.FRAME_NAME, this.parameterCount, FrameSlotKind.Object);
    this.frameSlots.put(CobolProgramStateNode.FRAME_NAME, frameSlot);

    CobolConstructorNode constructorNode =
        new CobolConstructorNode(
            programName,
            heap,
            inputParameterFileNames,
            this.firstFunctionName,
            this.fileLocalFunctions);
    CobolReadLocalVariableNode localVariableNode = CobolReadLocalVariableNodeGen.create(frameSlot);
  ...
  private RootCallTarget getCallTarget(int bodyEndPos) {
    ...
    final CobolRootNode rootNode =
        new CobolRootNode(
            language, this.frameDescriptor, sectionBody, functionSrc, this.functionName);
    return Truffle.getRuntime().createCallTarget(rootNode);
  }

  /**
   * Start a new function/section. todo: allow parameters? see
   * https://github.com/LoPoHa/CobolTruffle/issues/14 maybe only for the constructor above.
   *
  ...
  private CobolExpressionNode createAssignment(
      CobolStringLiteralNode nameNode, CobolExpressionNode valueNode, Integer argumentIndex) {
  ...
  /**
   * end the if, that is currently open. add the if as a statement to the statements of the parent.
   */
  void endIf() {
    CobolNodeFactoryBlock ifNode = this.currentBlock.getParent();
    assert ifNode != null;
    CobolStatementNode trueBlock = ifNode.getChilds().get(0).combineBlock();
    CobolStatementNode elseBlock =
        ifNode.getChilds().size() > 1 ? ifNode.getChilds().get(1).combineBlock() : null;
  ...
  private CobolExpressionNode createRead(Token token, CobolStringLiteralNode nameNode) {
    ...
    result.setSourceSection(token.getStartIndex(), token.getStopIndex() - token.getStartIndex());
    result.addExpressionTag();
    return result;
  }
