...
import com.github.lopoha.coboltruffle.parser.CobolBaseListener;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableDefinitionContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableDataTypeContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableValueNumberContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableValueStringContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableSizeContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableRedefinesContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableDataTypeStringContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.VariableDataTypeNumberContext;
import com.github.lopoha.coboltruffle.parser.CobolParser.IfConditionContext;
...
// todo: can a copy member don't start at level 01? and the following not declaration start at 01? if no, the preprocessor should put info in, when a copy starts/ends, so this can be checked...
...
public class CobolBaseListenerImpl extends CobolBaseListener {
    ...
    private int variableGetStringLength(VariableDataTypeStringContext ctx) {
        if (ctx.PICXS() != null) {
            ...
            return (sizeContext == null || sizeContext.NUMBER() == null) ? 1 : Integer.parseInt(sizeContext.NUMBER().getText());
        }
    }


    /// either returns null if no value was specified or a string with the length of the given size and the value right bound + filled with space if it is shorter.
    private String variableGetStringValue(VariableValueStringContext ctx, String variableName, int size) {
        // todo: cleanup
        if (ctx == null) {
            return null;
            ...
            if (removedQuotes.length() <= size) {
                ...
                throw new RuntimeException(String.format("The variable \"%s\" with the given length [%d] was too small for \"\" (length: [%d]", variableName, size, removedQuotes.length()));
            }
        }
    }
    ...
    /// either returns null if no value was specified or a string with the length of the given size and the value right bound + filled with space if it is shorter.
    private String variableGetNumberValue(VariableValueNumberContext ctx, String variableName, int size) {
        // todo: cleanup
        if (ctx == null) {
            return null;
        } else {
            ...
            if (string.length() <= size) {
                ...
                throw new RuntimeException(String.format("The variable \"%s\" with the given length [%d] was too small for \"\" (length: [%d]", variableName, size, string.length()));
            }
        }
    }
    ...
    private void addVariableDefinition(VariableDefinitionContext variableDefinitionContext, HeapBuilder heapBuilder) {
        ...
        if (variableDefinitionContext.FILLER() != null) {
            ...
            if (dataType == null) {
                final HeapBuilderVariable variable = new HeapBuilderVariable(level, null, HeapVariableType.Filler, redefines);
                ...
                if (dataTypeString != null) {
                    ...
                    final HeapBuilderVariable variable = new HeapBuilderVariable(level, null, HeapVariableType.Filler, size, value);
                    heapBuilder.add(variable);
            ...
            if (dataType == null) {
                ...
                final HeapBuilderVariable variable = new HeapBuilderVariable(level, variableName, HeapVariableType.None, redefines);
                heapBuilder.add(variable);
            } else {
                ...
                if (dataTypeString != null) {
                    final int size = variableGetStringLength(dataTypeString);
                    final String value = variableGetStringValue(dataTypeString.variableValueString(), variableName, size);
                    final HeapBuilderVariable variable = new HeapBuilderVariable(level, variableName, HeapVariableType.String, size, value);
                    heapBuilder.add(variable);
                } else if(dataTypeNumber != null) {
                    // todo: allow comma values e.g. 999.99
                    final int size = variableGetNumberSize(dataTypeNumber);
                    final String value = variableGetNumberValue(dataTypeNumber.variableValueNumber(), variableName, size);
                    final HeapBuilderVariable variable = new HeapBuilderVariable(level, variableName, HeapVariableType.Number, size, value);
                    heapBuilder.add(variable);
                } else {
                    throw new NotImplementedException();
                }
